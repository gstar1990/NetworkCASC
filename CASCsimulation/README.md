...
## [<img src="https://github.com/QuantLet/Styleguide-and-Validation-procedure/blob/master/pictures/qloqo.png" alt="Visit QuantNet">](http://quantlet.de/) **CASCsimulation** [<img src="https://github.com/QuantLet/Styleguide-and-Validation-procedure/blob/master/pictures/QN2.png" width="60" alt="Visit QuantNet 2.0">](http://quantlet.de/d3/ia)

```yaml

Name of QuantLet : CASCsimulation
...
T    = 10
L    = 4
s    = c(0, 1/50, 1/25, 1/20, 1/10, 1/5, 1/4, 1/2, 1)*N
nsim = 3
J    = 10

parallel.sim = function(K_0, N, T, L, sn, nsim, J){
    ...
    for(k in 1:nsim){
        R   = ceiling(log(N))
        Z_0 = array(0, c(N, K_0, T))
    
        rnd = ceiling(matrix(runif(N*T), N, T)*K_0)
        for(t in 1:T){
            for(i in 1:N){
                Z_0[i, rnd[i,t], t] = 1
            }
            if(t>1 &&  sn<=(N-1)){
                Z_0[(sn+1):N,,t] = Z_0[(sn+1):N,,t-1]
            }
            B[,,t] = matrix(c(0.9,0.6,0.3,0.6,0.3,0.4,0.3,0.4,0.8),3,3)*t/T
        }
        X = matrix(runif(N*R),N,R)*J
        ...
        for(t in 1:T){
            for( i in 1:length(perm)){
                s2[i,t] = sum(abs(Z_0[,perm[[i]],t]-Z_tilde[,,t]))/N/2
        
                s4[i,t] = sum(abs(Z_0[,perm[[i]],t]-Z_dsc4))/N/2
        
                s7[i,t] = sum(abs(Z_0[,perm[[i]],t]-Z_psk[,,t]))/N/2
        
                s8[i,t] = sum(abs(Z_0[,perm[[i]],t]-Z_C[,,t]))/N/2
            }
        }
...
res  = foreach(n=1:length(s)) %dopar% parallel.sim(K_0, N, T, L, sn=s[n], nsim, J)
stopCluster(cl)
...
#save.image("Sim_UdrtSwcMem.RData")


#jpeg("fig1.jpg", width=8, height=6, units="in", res=300)
plot(s, MisRate_ave_2, ylim=range(c(MisRate_ave_2, MisRate_ave_4,
                                    MisRate_ave_7, MisRate_ave_8)),
     typ="b", col="red", pch=8, lwd=1.5, lty="solid",
     ylab="Misclustering Rate", xlab="Number of Nodes Changed Membership")
lines(s, MisRate_ave_4, typ="b", col="blue", pch=15, lwd=1.5, lty="dashed")
lines(s, MisRate_ave_7, typ="b", col="green", pch=3, lwd=1.5, lty="dotted")
lines(s, MisRate_ave_8, typ="b", col="purple", pch=20, lwd=1.5, lty="dotdash")
legend("bottomright",c("CASC-DC","DSC-DC","DSC-PZ","DSC-Cw"),
